...
### 2026-02-17 - PII识别器优先级机制

#### 已完成任务

1. **优先级配置模块 (config/settings.py)**
   - 新增 `PIIPrioritySettings` 类：PII识别器优先级配置
   - 优先级规则：身份证(1) > 银行卡(2) > 手机号(3) > 护照(4) > 邮箱(5) > 姓名(6) > 地址(7)
   - 新增 `get_priority()` 方法：获取指定实体类型的优先级
   - 新增 `to_dict()` 方法：转换为字典

2. **分析器引擎更新 (core/analyzer.py)**
   - 新增 `_apply_priority_filter()` 方法：应用优先级过滤
   - 新增 `_results_overlap()` 方法：检测两个识别结果是否重叠
   - 修改 `analyze()` 方法：在置信度过滤后应用优先级过滤
   - 修改 `analyze_batch()` 方法：在置信度过滤后应用优先级过滤

3. **单元测试新增 (tests/unit/test_engines.py)**
   - 新增 `TestPIIPriorityFilter` 测试类：优先级过滤功能测试
   - 测试身份证优先级高于手机号
   - 测试身份证优先级高于银行卡
   - 测试银行卡优先级高于手机号
   - 测试不重叠实体不会被过滤
...
4. **TDD文档更新 (SPEC/TDD.md)**
   - 版本更新至v2.0
   - 新增"2.5 PII识别器优先级机制"章节
   - 包含设计背景、优先级规则、实现设计、处理流程和示例

#### 技术变更说明

| 变更项 | 变更前 | 变更后 |
|--------|--------|--------|
| 识别结果处理 | 无优先级机制，可能产生重叠结果 | 添加优先级过滤，确保重叠结果只保留高优先级类型 |
| 身份证识别 | 可能同时被识别为银行卡/手机号 | 优先级最高，覆盖其他重叠结果 |
| 银行卡识别 | 可能同时被识别为手机号 | 优先级次高，覆盖手机号重叠结果 |
...
### 2026-02-17 - 姓名识别IE schema扩展

#### 已完成任务

1. **信息抽取引擎更新 (nlp/ie_engine.py)**
   - 新增 `NAME_SCHEMA_KEYS` 类常量：`{"姓名", "人名"}`
   - 修改 `extract_names` 方法：
     - 支持识别"姓名"和"人名"两种schema类型的姓名信息
     - 动态检测当前schema中包含的姓名类型key
     - 遍历所有姓名类型key提取姓名信息
   - 更新 `DEFAULT_SCHEMA` 为 `["地址", "具体地址", "姓名", "人名"]`

2. **姓名识别器更新 (recognizers/name_recognizer.py)**
   - 新增 `NAME_SCHEMA_KEYS` 类常量：`{"姓名", "人名"}`
   - 修改缓存处理逻辑，支持从"姓名"和"人名"两种key提取姓名信息

3. **分析器引擎更新 (core/analyzer.py)**
   - 更新IE引擎schema为 `["地址", "具体地址", "姓名", "人名"]`
...
| 变更项 | 变更前 | 变更后 |
|--------|--------|--------|
| 姓名schema支持 | 仅支持"姓名" | 支持"姓名"和"人名" |
| DEFAULT_SCHEMA | ["地址", "具体地址", "姓名"] | ["地址", "具体地址", "姓名", "人名"] |
| schema检查方式 | 硬编码检查"姓名" | 使用NAME_SCHEMA_KEYS集合动态检查 |
| 姓名提取逻辑 | 仅提取"姓名"key | 遍历所有姓名类型key提取 |
